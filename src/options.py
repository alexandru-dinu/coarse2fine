def set_model_options(parser):
    ...
    parser.add_argument('-parent_feed_hidden', type=int, default=0, help='Hidden size for parent feed vector.')

    parser.add_argument('-layers', type=int, default=1, help='Number of layers in enc/dec.')
    parser.add_argument('-enc_layers', type=int, default=1, help='Number of layers in the encoder')
    parser.add_argument('-dec_layers', type=int, default=1, help='Number of layers in the decoder')

    parser.add_argument('-rnn_size', type=int, default=300, help='Size of LSTM hidden states')

    parser.add_argument('-rnn_type', type=str, default='LSTM', choices=['LSTM', 'GRU'], help="The gate type to use in the RNNs")
    parser.add_argument('-brnn_merge', default='concat', choices=['concat', 'sum'], help="Merge action for the bidir hidden states")

    # Attention options
    parser.add_argument('-global_attention', type=str, default='general', choices=['dot', 'general', 'mlp'],
        help="The attention type to use: dotprot or general (Luong) or MLP (Bahdanau)"
    )
    parser.add_argument('-attn_hidden', type=int, default=-1, help="if attn_hidden > 0, then attention score = f(Ue) B f(Ud)")

    # Layout options
    parser.add_argument('-layout_token_prune', action="store_true", help="Predict whether a token appears in the layout sequence.")

    # Target options
    parser.add_argument('-no_share_emb_layout_encoder', action="store_true", help='Whether share embeddings for layout encoder.')
    parser.add_argument('-mask_target_loss', action="store_true", help='Whether mask target sequence loss.')
    parser.add_argument('-lay_co_attention', action="store_true", help='Use co-attention for layout encoder towards input.')
    parser.add_argument('-q_co_attention', action="store_true", help='Use co-attention for input encoder towards layout.')
    parser.add_argument('-copy_prb', type=str, default='hidden', choices=['hidden', 'hidden_context'],
        help="How to compute prb(copy). hidden: rnn hidden vector; hidden_context: also use attention context vector;"
    )

    # Ablation
    parser.add_argument('-no_lay_encoder', action="store_true", help='No layout RNN encoder.')
...
def set_preprocess_options(parser):
    ...
    parser.add_argument('-src_seq_length_trunc', type=int, default=0, help="Truncate source sequence length.")
    parser.add_argument('-tgt_seq_length', type=int, default=50, help="Maximum target sequence length to keep.")
    parser.add_argument('-tgt_seq_length_trunc', type=int, default=0, help="Truncate target sequence length.")

    # Data processing options
    parser.add_argument('-shuffle', type=int, default=1, help="Shuffle data")
    parser.add_argument('-permute_order', type=int, default=0, help="Permute order for logical forms for data augumentation.")
...
def set_train_options(parser):
    ...
    parser.add_argument('-fix_word_vecs', action='store_true', help="Fix word embeddings on the encoder side.")
    ...
    parser.add_argument('-dropout_i', type=float, default=0.5, help="Dropout rate (for RNN input).")
    parser.add_argument('-lock_dropout', action='store_true', help="Use the same dropout mask for RNNs.")
    parser.add_argument('-weight_dropout', type=float, default=0, help=">0: Weight dropout probability; applied in LSTM stacks.")
    parser.add_argument('-dropword_enc', type=float, default=0, help="Drop word rate.")
    parser.add_argument('-dropword_dec', type=float, default=0, help="Drop word rate.")
    parser.add_argument('-smooth_eps', type=float, default=0, help="Label smoothing")
    parser.add_argument('-moving_avg', type=float, default=0, help="Exponential moving average")

    # learning rate
    parser.add_argument('-learning_rate', type=float, default=0.005, help="Starting learning rate.")
    parser.add_argument('-alpha', type=float, default=0.95, help="Optimization hyperparameter")
...
def set_translation_options(parser):
    ...
    parser.add_argument('-run_from', type=int, default=0, help='Only evaluate run.* >= run_from.')
